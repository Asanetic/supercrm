...
const moduleApi =  '/api/supererpv5/companies';

export const CompaniesSchema = {
  entity: 'crm_companies',              // DB table name; also drives default role names
  ...
  importDataEndpoint : '/api/supererpv5/companies/import',
  ...
  showInList: ['row_count', 'company_logo','company_name', 'industry', 'company_email', 'company_phone', 'website', 'annual_revenue', 'tax_number'],

  //export columns these columns are used to generate upload csv template file
  exportColumns: ['row_count', 'company_name', 'industry', 'company_email', 'company_phone', 'website', 'annual_revenue', 'tax_number'],
  ...
  profileActions: [
    { key: 'back', label: 'Back to list', icon: 'arrow-left', variant: 'outline-secondary', navigateTo: '/supererpv5/companies/list', grid: false, form: true },
    { key: 'save', label: 'Save', icon: 'save', variant: 'primary', grid: false, form: true, rowAction: false },
    { key: 'delete', label: 'Delete', icon: 'trash', variant: 'outline-danger', confirm: 'Are you sure you want to delete this companie?', editOnly: true, grid: false, form: true, rowAction: true, role: 'manage_companies' },
    { key: 'view', label: 'View more', icon: 'edit', rowAction: true },
    { key: 'new', label: 'New Companie', icon: 'plus', variant: 'outline-primary', navigateTo: '/supererpv5/companies/profile', grid: true, form: false, rowAction: false },
    { key: 'clone', label: 'Clone Record', icon: 'copy', colorClass: 'dyn-btn-accent-blue', editOnly: true, grid: false, form: true, rowAction: false, role: 'manage_companies' },
  ...
  sections: [
    { key: 'basic_information', label: 'Basic Information', columns: 3, fields: ['company_logo','company_name', 'industry', 'company_email', 'company_phone', 'website', 'kra_pin', 'tax_number'] },
    { key: 'other_details', label: 'Other Details', columns: 3, fields: ['country', 'city', 'company_size', 'annual_revenue', 'date_created', 'address', 'notes'] },
  ],
  ...
  fields: [
    ...
    { key: 'industry', label: 'Industry', type: 'groupedSelect', endpoint:moduleApi, searchable: true },
    ...
    { key: 'country', label: 'Country', type: 'groupedSelect', endpoint:moduleApi, groupByField: 'country', searchable: true },
    { key: 'city', label: 'City', type: 'text', type: 'groupedSelect', endpoint:moduleApi, },
    { key: 'address', label: 'Address', type: 'textarea', colSpan: 6, searchable: true },
    { key: 'company_logo', label: 'Company Logo', type: 'image' , imageClass:"useravatar_120", colSpan:12},
    { key: 'company_size', label: 'Company Size', type: 'groupedSelect', endpoint:moduleApi, searchable: true },
    { key: 'annual_revenue', label: 'Annual Revenue', type: 'money', sum:true },
