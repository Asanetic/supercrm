...
// generic keys (full_name / phone_number / email / record_id) — a
// messages row carries them as recipient_name / recipient_phone /
// recipient_email / related_record_id.
...
const MessagesActions = {
  ...
  // Opens the shared Smart Call picker for this message's recipient.
  call: ({ rows }) => {
    const row = rows?.[0];
    if (!row) return;
  ...
  // Re-opens the composer with the SAME subject/content and the SAME
  // recipient prefilled — review then hit Send; creates a fresh
  // smart_messages row, doesn't edit this one.
  resend_message: ({ rows }) => {
    const row = rows?.[0];
    if (!row) return;
    MosySendSmartMessage({
      profileDataNode: toRecipient(row),
      uiOptions: {
  ...
  // Same subject/content prefilled, but the recipient fields are left
  // blank (only related_record_id carries over — as `record_id`, the key
  // sendViaSelectedMode in MosySmartComms.jsx actually reads) — the user
  // picks who to forward it to.
  forward_message: ({ rows }) => {
    const row = rows?.[0];
    if (!row) return;
    MosySendSmartMessage({
      profileDataNode: { record_id: row.related_record_id },
      uiOptions: {
