...
const moduleApi = apiRoutes.notes.base;

export const NotesSchema = {
  entity: 'notes_ideas',              // DB table name; also drives default role names
                                     // (view_notes / manage_notes) and apiBase
  label: 'Notes',                 // optional, defaults to entity capitalized
  apiBase: moduleApi,

  //api endpint for importing data from csv
  importDataEndpoint : apiRoutes.notes.import,
  ...
  profileActions: [
    { key: 'back', label: 'Back to list', icon: 'arrow-left', variant: 'outline-secondary', navigateTo: '/imsv2/notes/list', grid: false, form: true },
    { key: 'save', label: 'Save', icon: 'save', variant: 'primary', grid: false, form: true, rowAction: false },
    { key: 'delete', label: 'Delete', icon: 'trash', variant: 'outline-danger', confirm: 'Are you sure you want to delete this note?', editOnly: true, grid: false, form: true, rowAction: true, role: 'manage_notes' },
  ...
  showInList: ['row_count', 'contact_name', 'deal', 'type', 'title', 'content', 'status', 'name'],

  //export columns these columns are used to generate upload csv template file
  exportColumns: ['contact_id', 'opportunity_id', 'type', 'title', 'content', 'status'],

  sections: [
    { key: 'basic_information', label: 'Basic Information', columns: 3, fields: ['title','contact_id', 'opportunity_id', 'type', 'status', 'content'] },
  ],
  ...
  fields: [
    ...
    { key: 'note_id', label: 'Note Id', type: 'text', system: true, editable: false },
