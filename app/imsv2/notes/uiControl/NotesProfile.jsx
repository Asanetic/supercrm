...
// NotesProfile — pure shell. It resolves the id, wires up the
...
export default function NotesProfile({ id: idProp, onDone, hiddenActions = [], presetValues, schemaOverride }) {
  const searchParams = useSearchParams();
  const schema = schemaOverride || NotesSchema;
  const id = idProp ?? searchParams.get(`${NotesSchema.entity}_dataNode`);
  const form = useEntityFormController(schema, NotesActions, {
    id,
    onDone,
