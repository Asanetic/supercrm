...
const moduleApi = apiRoutes.ledger.base;

export const LedgerSchema = {
  entity: 'ledger',              // DB table name; also drives default role names
                                     // (view_ledger / manage_ledger) and apiBase
  label: 'Ledger',                 // optional, defaults to entity capitalized
  apiBase: moduleApi,

  //api endpint for importing data from csv
  importDataEndpoint : apiRoutes.ledger.import,
  ...
  profileActions: [
    { key: 'back', label: 'Back to list', icon: 'arrow-left', variant: 'outline-secondary', navigateTo: '/imsv2/ledger/list', grid: false, form: true },
    { key: 'save', label: 'Save', icon: 'save', variant: 'primary', grid: false, form: true, rowAction: false },
    { key: 'delete', label: 'Delete', icon: 'trash', variant: 'outline-danger', confirm: 'Are you sure you want to delete this ledger?', editOnly: true, grid: false, form: true, rowAction: true, role: 'manage_ledger' },
    { key: 'view', label: 'View more', icon: 'edit', rowAction: true },
    { key: 'new', label: 'New Ledger', icon: 'plus', variant: 'outline-primary', navigateTo: '/imsv2/ledger/profile', grid: true, form: false, rowAction: false },
    { key: 'clone', label: 'Clone Record', icon: 'copy', variant: 'outline-secondary', editOnly: true, grid: false, form: true, rowAction: false, role: 'manage_ledger' },
  ...
  showInList: ['row_count', 'contact_name', 'title', 'type', 'amount', 'currency', 'transaction_date', 'due_date'],

  //export columns these columns are used to generate upload csv template file
  exportColumns: ['contact_id', 'opportunity_id', 'type', 'amount', 'currency', 'transaction_date', 'due_date'],

  sections: [
    { key: 'basic_information', label: 'Basic Information', columns: 3, fields: ['contact_id', 'opportunity_id', 'type', 'amount', 'currency', 'transaction_date', 'due_date'] },
    { key: 'other_details', label: 'Other Details', columns: 3, fields: ['reference', 'description', 'status', 'reg_date'] },
  ],
  ...
  fields: [
    ...
    { key: 'ledger_id', label: 'Ledger Id', type: 'text', system: true, editable: false },
...
// dyn-btn-accent-teal   { border-color: transparent; color: #0f766e; }
  ...
  // roles: { view: 'view_ledger', manage: 'manage_ledger' },
};
