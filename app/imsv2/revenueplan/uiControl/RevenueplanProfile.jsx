...
export default function RevenueplanProfile({ id: idProp, onDone, hiddenActions = [], presetValues, schemaOverride }) {
  const searchParams = useSearchParams();
  const schema = schemaOverride || RevenueplanSchema;
  const id = idProp ?? searchParams.get(`${RevenueplanSchema.entity}_dataNode`);
  const form = useEntityFormController(schema, RevenueplanActions, {
    id,
    onDone,
