...
// IncomesourcesProfile — pure shell. It resolves the id, wires up the
...
export default function IncomesourcesProfile({ id: idProp, onDone, hiddenActions = [], presetValues, schemaOverride }) {
  const searchParams = useSearchParams();
  const schema = schemaOverride || IncomesourcesSchema;
  const id = idProp ?? searchParams.get(`${IncomesourcesSchema.entity}_dataNode`);
  const form = useEntityFormController(schema, IncomesourcesActions, {
    id,
    onDone,
