...
const moduleApi = apiRoutes.incomesources.base;

export const IncomesourcesSchema = {
  entity: 'income_sources',              // DB table name; also drives default role names
                                     // (view_incomesources / manage_incomesources) and apiBase
  label: 'Income sources',                 // optional, defaults to entity capitalized
  apiBase: moduleApi,

  //api endpint for importing data from csv
  importDataEndpoint : apiRoutes.incomesources.import,
  ...
  profileActions: [
    { key: 'back', label: 'Back to list', icon: 'arrow-left', variant: 'outline-secondary', navigateTo: '/imsv2/incomesources/list', grid: false, form: true },
    { key: 'save', label: 'Save', icon: 'save', variant: 'primary', grid: false, form: true, rowAction: false },
    { key: 'delete', label: 'Delete', icon: 'trash', variant: 'outline-danger', confirm: 'Are you sure you want to delete this incomesource?', editOnly: true, grid: false, form: true, rowAction: true, role: 'manage_incomesources' },
    { key: 'view', label: 'View more', icon: 'edit', rowAction: true },
    { key: 'new', label: 'Add source', icon: 'plus', variant: 'outline-primary', navigateTo: '/imsv2/incomesources/profile', grid: true, form: false, rowAction: false },
    { key: 'clone', label: 'Clone Record', icon: 'copy', variant: 'outline-secondary', editOnly: true, grid: false, form: true, rowAction: false, role: 'manage_incomesources' },
  ...
  showInList: ['row_count', 'name', 'income_type', 'niche', 'default_amount', 'description'],

  //export columns these columns are used to generate upload csv template file
  exportColumns: ['name', 'description', 'income_type', 'niche', 'default_amount', 'currency', 'status'],

  sections: [
    { key: 'basic_information', label: 'Basic Information', columns:4, fields: ['name',  'income_type', 'niche', 'default_amount', 'description', 'currency', 'status', 'reg_date'] },
   ],
  ...
  fields: [
    ...
    { key: 'income_source_id', label: 'Income Source Id', type: 'text', system: true, editable: false },
    { key: 'name', label: 'Name', type: 'text', title: true, required: true , colSpan : 9},
    { key: 'description', label: 'Description', type: 'textarea', colSpan: 12 },
    { key: 'income_type', label: 'Tag', type: 'groupedSelect', endpoint: moduleApi, groupByField: 'income_type' },
    { key: 'niche', label: 'Niche', type: 'groupedSelect', endpoint: moduleApi, groupByField: 'niche' , title :true},
    { key: 'default_amount', label: 'Default Amount', type: 'money' , sum :true },
...
// dyn-btn-accent-teal   { border-color: transparent; color: #0f766e; }
  ...
  // roles: { view: 'view_incomesources', manage: 'manage_incomesources' },
};
