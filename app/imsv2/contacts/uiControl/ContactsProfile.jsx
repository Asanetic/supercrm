...
export default function ContactsProfile({ id: idProp, onDone, hiddenActions = [], presetValues, schemaOverride }) {
  const searchParams = useSearchParams();
  const schema = schemaOverride || ContactsSchema;
  const id = idProp ?? searchParams.get(`${ContactsSchema.entity}_dataNode`);
  const form = useEntityFormController(schema, ContactsActions, {
    id,
    onDone,
