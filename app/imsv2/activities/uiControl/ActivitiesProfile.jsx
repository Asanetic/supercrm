...
export default function ActivitiesProfile({ id: idProp, onDone, hiddenActions = [], presetValues, schemaOverride }) {
  const searchParams = useSearchParams();
  const schema = schemaOverride || ActivitiesSchema;
  const id = idProp ?? searchParams.get(`${ActivitiesSchema.entity}_dataNode`);
  const form = useEntityFormController(schema, ActivitiesActions, {
    id,
    onDone,
