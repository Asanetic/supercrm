...
export default function SystemusersProfile({ id: idProp, onDone, hiddenActions = [], presetValues, schemaOverride }) {
  const searchParams = useSearchParams();
  const schema = schemaOverride || SystemusersSchema;
  const id = idProp ?? searchParams.get(`${SystemusersSchema.entity}_dataNode`);
  const form = useEntityFormController(schema, SystemusersActions, {
    id,
    onDone,
