...
// PaymenthistoryProfile — pure shell. It resolves the id, wires up the
...
export default function PaymenthistoryProfile({ id: idProp, onDone, hiddenActions = [], presetValues, schemaOverride }) {
  const searchParams = useSearchParams();
  const schema = schemaOverride || PaymenthistorySchema;
  const id = idProp ?? searchParams.get(`${PaymenthistorySchema.entity}_dataNode`);
  const form = useEntityFormController(schema, PaymenthistoryActions, {
    id,
    onDone,
