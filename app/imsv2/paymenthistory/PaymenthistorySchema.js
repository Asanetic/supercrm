...
const moduleApi = apiRoutes.paymenthistory.base;

export const PaymenthistorySchema = {
  entity: 'payment_history',              // DB table name; also drives default role names
                                     // (view_paymenthistory / manage_paymenthistory) and apiBase
  label: 'Payment history',                 // optional, defaults to entity capitalized
  apiBase: moduleApi,

  //api endpint for importing data from csv
  importDataEndpoint : apiRoutes.paymenthistory.import,
  ...
  moduleRole: 'view_paymenthistory',
  ...
  profileActions: [
    { key: 'back', label: 'Back to list', icon: 'arrow-left', variant: 'outline-secondary', navigateTo: '/imsv2/paymenthistory/list', grid: false, form: true },
    { key: 'save', label: 'Save', icon: 'save', variant: 'primary', grid: false, form: true, rowAction: false },
    { key: 'delete', label: 'Delete', icon: 'trash', variant: 'outline-danger', confirm: 'Are you sure you want to delete this paymenthistory?', editOnly: true, grid: false, form: true, rowAction: true, role: 'manage_paymenthistory' },
    { key: 'view', label: 'View more', icon: 'edit', rowAction: true },
    { key: 'new', label: 'Log Payment', icon: 'plus', variant: 'outline-primary', navigateTo: '/imsv2/paymenthistory/profile', grid: true, form: false, rowAction: false },
    { key: 'clone', label: 'Clone Record', icon: 'copy', variant: 'outline-secondary', editOnly: true, grid: false, form: true, rowAction: false, role: 'manage_paymenthistory' },
    ...
    // Related-record popups — routed through logicControl/actionsRegistry.js.
    { key: 'view_expected_revenue', label: 'Expected Revenue', icon: 'line-chart', variant: 'outline-secondary', type: 'action', editOnly: true, grid: false, form: true, rowAction: true },
    { key: 'view_payment_request', label: 'Payment Request', icon: 'money', variant: 'outline-secondary', type: 'action', editOnly: true, grid: false, form: true, rowAction: true },

    // Pops the shared Smart Messenger, prefilled with a receipt message
    // addressed to this payment's payer (contact_id).
    { key: 'send_payment_receipt', label: 'Send Receipt', icon: 'paper-plane', variant: 'outline-info', type: 'action', editOnly: true, grid: false, form: true, rowAction: true },
  ],
  ...
  showInList: ['row_count', 'payment_date', 'contact_name', 'title', 'amount', 'payment_reference',  'payment_method'],

  //export columns these columns are used to generate upload csv template file
  exportColumns: ['expected_income_id', 'contact_id', 'opportunity_id', 'amount', 'payement_reference', 'payment_date', 'payment_method'],

  sections: [
    { key: 'basic_information', label: 'Basic Information', columns: 4, fields: ['expected_income_id', 'contact_id', 'opportunity_id', 'amount', 'payment_date', 'payment_method','payment_reference'] },
    { key: 'other_details', label: 'Other Details', columns: 3, fields: ['description', 'status', 'reg_date'] },
  ],
  ...
  fields: [
    ...
    { key: 'payment_id', label: 'Payment Id', type: 'text', system: true, editable: false },
    ...resolveField('expected_income_id', { as: 'title',  colSpan: 4 }),
    ...
    { key: 'payment_method', label: 'Payment Mode', type: 'groupedSelect', endpoint: moduleApi, groupByField: 'payment_method' },
    ...
    { key: 'reg_date', label: 'Record Date', type: 'datetime' , editable: false , editOnly: true},
