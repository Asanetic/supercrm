...
const moduleApi = apiRoutes.opportunities.base;

export const OpportunitiesSchema = {
  entity: 'opportunities',              // DB table name; also drives default role names
                                     // (view_opportunities / manage_opportunities) and apiBase
  label: 'Deals',                 // optional, defaults to entity capitalized
  apiBase: moduleApi,

  //api endpint for importing data from csv
  importDataEndpoint : apiRoutes.opportunities.import,
  ...
  profileActions: [
    { key: 'back', label: 'Back to list', icon: 'arrow-left', variant: 'outline-secondary', navigateTo: '/imsv2/opportunities/list', grid: false, form: true },
    { key: 'save', label: 'Save', icon: 'save', variant: 'primary', grid: false, form: true, rowAction: false },
    { key: 'delete', label: 'Delete', icon: 'trash', variant: 'outline-danger', confirm: 'Are you sure you want to delete this opportunitie?', editOnly: true, grid: false, form: true, rowAction: true, role: 'manage_opportunities' },
    { key: 'view', label: 'View more', icon: 'edit', rowAction: true },
    { key: 'new', label: 'Add Deal', icon: 'plus', variant: 'outline-primary', navigateTo: '/imsv2/opportunities/profile', grid: true, form: false, rowAction: false },
    { key: 'clone', label: 'Clone Record', icon: 'copy', variant: 'outline-secondary', editOnly: true, grid: false, form: true, rowAction: false, role: 'manage_opportunities' },
    ...
    // Grid-toolbar smart filters — status/stage pick from this table's own
    // distinct values, client scopes by contact_id via the Contacts module,
    // date range-filters on the expected close date.
    ...
    { key: 'filter_by_stage', label: 'Filter by Stage', icon: 'flag', variant: 'outline-secondary', type: 'action', grid: true, form: false, rowAction: false },
  ...
  sections: [
    ...
    { key: 'other_details', label: 'Other Details', columns: 2, fields: ['status', 'reg_date'] },
  ],
  ...
  fields: [
    ...
    { key: 'opportunity_id', label: 'Opportunity Id', type: 'text', system: true, editable: false },
    ...resolveField('contact_id', { as: 'contact_name', colSpan:4 , required: true }),
    { key: 'title', label: 'Title', type: 'text', colSpan: 4, title: true, required: true },
    { key: 'description', label: 'Description', type: 'textarea', colSpan: 12 },
    { key: 'type', label: 'Type', type: 'select', colSpan: 4, options: ['New Business', 'Existing Business', 'Renewal', 'Upsell'] },
