...
const moduleApi = apiRoutes.mosymetrics.base;

export const MosymetricsSchema = {
  entity: 'mosy_metrics',              // DB table name; also drives default role names
                                     // (view_mosymetrics / manage_mosymetrics) and apiBase
  label: 'Mosymetrics',                 // optional, defaults to entity capitalized
  apiBase: moduleApi,

  //api endpint for importing data from csv
  importDataEndpoint : apiRoutes.mosymetrics.import,
  ...
  profileActions: [
    { key: 'back', label: 'Back to list', icon: 'arrow-left', variant: 'outline-secondary', navigateTo: '/imsv2/mosymetrics/list', grid: false, form: true },
    { key: 'save', label: 'Save', icon: 'save', variant: 'primary', grid: false, form: true, rowAction: false },
    { key: 'delete', label: 'Delete', icon: 'trash', variant: 'outline-danger', confirm: 'Are you sure you want to delete this mosymetric?', editOnly: true, grid: false, form: true, rowAction: true, role: 'manage_mosymetrics' },
    { key: 'view', label: 'View more', icon: 'edit', rowAction: true },
    { key: 'new', label: 'New Mosymetric', icon: 'plus', variant: 'outline-primary', navigateTo: '/imsv2/mosymetrics/profile', grid: true, form: false, rowAction: false },
    { key: 'clone', label: 'Clone Record', icon: 'copy', variant: 'outline-secondary', editOnly: true, grid: false, form: true, rowAction: false, role: 'manage_mosymetrics' },
  ...
  showInList: ['row_count', 'app_name', 'metric_key', 'label', 'sql_query', 'db_connection_key', 'status', 'reg_date'],

  //export columns these columns are used to generate upload csv template file
  exportColumns: ['app_name', 'metric_key', 'label', 'sql_query', 'db_connection_key', 'status', 'reg_date'],

  sections: [
    { key: 'basic_information', label: 'Basic Information', columns: 3, fields: ['app_name', 'metric_key', 'label', 'sql_query', 'db_connection_key', 'status', 'reg_date'] },
  ],
  ...
  fields: [
    ...
    { key: 'metric_id', label: 'Metric Id', type: 'text', system: true, editable: false },
    { key: 'app_name', label: 'App Name', type: 'text', title: true, required: true },
    { key: 'metric_key', label: 'Metric Key', type: 'text' },
    { key: 'label', label: 'Label', type: 'text' },
    { key: 'sql_query', label: 'Sql Query', type: 'textarea' },
    { key: 'db_connection_key', label: 'Db Connection Key', type: 'text' },
