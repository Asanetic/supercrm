'use client';

//React
import { useEffect, useState } from 'react';

import Link from 'next/link';

import { useRouter } from 'next/navigation';
//access control
import {MosyAccessControl} from "../../UiControl/MosyAccessControl"
import {MosyUIGuard } from "../../UiControl/MosyUiGuard"


//components
import { MosyAlertCard, MosyNotify ,closeMosyModal } from  '../../../MosyUtils/ActionModals';
import MosySnackWidget from '../../../MosyUtils/MosySnackWidget';

//basic utils
import { mosyScrollTo , deleteUrlParam, mosyFormInputHandler,mosyUrlParam ,mosyTonum  } from '../../../MosyUtils/hiveUtils';

//data control and processors
import { inteprateUserManifestFormAction, userManifestProfileData , popDeleteDialog, InteprateUserManifestEvent } from '../dataControl/UserManifestRequestHandler';

//state management
import { useUserManifestState } from '../dataControl/UserManifestStateManager';

//profile components
import {
  SubmitButtons,
  AddNewButton,
  LiveSearchDropdown,
  MosySmartField,
  MosyActionButton,
  SmartDropdown,
  DeleteButton ,
  MosyImageViewer,
  MosyFileUploadButton
} from '../../UiControl/componentControl';

//def logo
import logo from '../../../img/logo/logo.png'; // outside public!

import MosyHtmlEditor from '../../../MosyUtils/htmlEditor'

//routes manager
///handle routes
import { getApiRoutes } from '../../AppRoutes/apiRoutesHandler';

// Use default base root (/)
const apiRoutes = getApiRoutes();

import {InteprateSystemUsersEvent} from '../../systemusers/dataControl/SystemUsersRequestHandler';
import SystemUsersList from '../../systemusers/uiControl/SystemUsersList';
import SystemUsersProfile from '../../systemusers/uiControl/SystemUsersProfile';
// ════════════════════════════════════════════════════════════════
// PROFILE PAGE FUNCTION IMPORTS
// ════════════════════════════════════════════════════════════════
// Imports from system_users-automapper.jsx
import {
  viewSystemUsers
} from '../../systemusers/logicControl/system_users-automapper';



// export profile

//import minilist component manager
import { MosyProfileSection} from '../../UiControl/dataMapUiControl';


///component access control key
export const MOSY_ACCESS_KEY = "MANAGE_USER_MANIFEST_";

//live data detial / profile component

export default function UserManifestProfile({ dataIn = {}, dataOut = {} }) {
  
  //initiate data exchange manifest
  //incoming data from parent
  const {
    showNavigationIsle = true,
    customQueryStr = "",
    backToList="./list",
    parentUseEffectKey = "",
    parentStateSetters=null,
    customProfileData={},
    hostParent="UserManifestMainProfilePage",
    parentProfileItemId = "UserManifestProfileTray"
    
  } = dataIn;
  
  //outgoing data to parent
  const {
    setChildDataOut = () => {},
    setChildDataOutSignature = () => {},
  } = dataOut;
  
  
  //set default state values
  const settersOverrides  = {localEventSignature : parentUseEffectKey,   activeScrollId : parentProfileItemId}
  
  //manage UserManifest states
  const [stateItem, stateItemSetters] = useUserManifestState(settersOverrides);
  const user_manifest_Node = stateItem.userManifestNode
  
  // -- basic states --//
  const paramUserManifestUptoken  = stateItem.userManifestUptoken
  const userManifestActionStatus = stateItem.userManifestActionStatus
  const snackMessage = stateItem.snackMessage
  const activeScrollId = stateItem.activeScrollId
  
  //const snackOnDone = stateItem.snackOnDone
  
  const localEventSignature = stateItem.localEventSignature
  
  const handleInputChange = mosyFormInputHandler(stateItemSetters.setUserManifestNode);
  
  //use route navigation system
  const router = useRouter();
  
  //manage post form
  function postUserManifestFormData(e) {
    
    MosyNotify({message: "Sending request",icon:"send"})
    
    inteprateUserManifestFormAction(e, stateItemSetters).then(response=>{
      
      setChildDataOut({
        
        actionName : response.actionName,
        dataToken : response.newToken,
        actionsSource : "postUserManifestFormData",
        setters :{
          
          childStateSetters: stateItemSetters,
          parentStateSetters: parentStateSetters
          
        }
        
      })
      
      //focus on this form on submission
      stateItemSetters.setActiveScrollId("UserManifestProfileTray")
      mosyScrollTo(activeScrollId)
      
      closeMosyModal()
      
    })
    
  }
  
  useEffect(() => {
    
    userManifestProfileData(customQueryStr, stateItemSetters, router, customProfileData)
    
    mosyScrollTo(activeScrollId)
    
  }, [localEventSignature]);
  
  
  
  //child queries use effect
  
  //setSystemUsersCustomProfileQuery Script
  const setSystemUsersCustomProfileQuery = stateItemSetters.setSystemUsersCustomProfileQuery;
  const systemUsersCustomProfileQuery =  stateItem.systemUsersCustomProfileQuery;
  
  useEffect(() => {
    if (user_manifest_Node?.primkey && setSystemUsersCustomProfileQuery) {
      
      const query = {recordId:btoa(user_manifest_Node?.user_id)    };
      
      const tokenUrl = mosyUrlParam("system_users_dataNode")
      
      if(!tokenUrl)
      {
        setSystemUsersCustomProfileQuery(query);
      }
      
    }
  }, [user_manifest_Node, setSystemUsersCustomProfileQuery]);
  
  
  //access control managemant
  const [allowed, setAllowed] = useState(null);
  
  useEffect(() => {
    setAllowed(MosyAccessControl(MOSY_ACCESS_KEY));
  }, []);
  
  if (allowed === null) return null;
  if (!allowed) return <MosyUIGuard />;
  
  return (
    
    <div className="p-0 col-md-12 text-center row justify-content-center m-0  " id="UserManifestProfileTray">
      {/* ================== Start Feature Section========================== ------*/}
      
      
      <div className="col-md-12 rounded text-left p-2 mb-0  bg-white ">
        <div className={` profile_container col-md-12 m-0 p-0  ${showNavigationIsle &&("pr-lg-4 pl-lg-4 m-0")}`}>
          <form onSubmit={postUserManifestFormData} encType="multipart/form-data" id="user_manifest__profile_form">
            
            {/*    Title isle      */}
            <div className="col-md-12 pt-4 p-0 hive_profile_title_top d-lg-none" id=""></div>
            <h3 className="col-md-12 title_text text-left p-0 pt-3 hive_profile_title row justify-content-center m-0 ">
              <div className="col m-0 p-0 pb-3">
                
                {
                  user_manifest_Node?.primkey ? (
                    
                    <span>{`User Manifest  / ${user_manifest_Node?.user_name}`}</span>
                    
                  ) : customProfileData?.UserManifestTitle ? (
                    
                    <span>{customProfileData.UserManifestTitle}</span>
                    
                  ) : (
                    
                    <span>New User Manifest </span>
                    
                  )}
                  
                </div>
                <>{!showNavigationIsle && (<div className="col m-0 p-0 text-right ">
                  {paramUserManifestUptoken && (
                    <DeleteButton
                    src="UserManifestMainProfilePage"
                    tableName="user_manifest_"
                    uptoken={paramUserManifestUptoken}
                    stateItemSetters={stateItemSetters}
                    parentStateSetters={parentStateSetters}
                    
                    onDelete={popDeleteDialog}
                    />
                  )}
                </div>)}</>
              </h3>
              {/*    Title isle      */}
              
              
              
              {/*    Navigation isle      */}
              <><div className="row justify-content-end m-0 p-0 col-md-12  p-3  hive_profile_navigation " id="">
                <div className="col-md-4 text-left p-0 hive_profile_nav_back_to_list_tray" id="">
                  
                  {showNavigationIsle && (
                    <>
                    <Link href={backToList} className="text-info hive_profile_nav_back_to_list "><i className="fa fa-arrow-left"></i> Back to list</Link>
                  </>
                )}
                
              </div>
              <div className="col-md-8 p-0 text-right hive_profile_nav_add_new_tray" id="">
                
                
                
                {paramUserManifestUptoken && (
                  <>
                  
                  <MosyActionButton
                  source="UserManifestProfile"
                  action="view_user_details_profile_action_btn"
                  label="View User Details"
                  icon="list"
                  
                  onClick={()=>{
                    
                    viewSystemUsers({childCol:`recordId`,parentColVal:user_manifest_Node.user_id,parentName:user_manifest_Node.user_name})
                    
                  }}
                  />
                </>
              )}
              
              {paramUserManifestUptoken && showNavigationIsle && (
                <>
                
                <DeleteButton
                src="UserManifestMainProfilePage"
                tableName="user_manifest_"
                uptoken={paramUserManifestUptoken}
                stateItemSetters={stateItemSetters}
                parentStateSetters={parentStateSetters}
                router={router}
                onDelete={popDeleteDialog}
                />
                
                
                <AddNewButton
                src="UserManifestMainProfilePage"
                tableName="user_manifest_"
                link="./profile"
                label="New User Manifest "
                icon="plus-circle" />
              </>
            )}
            
          </div>
        </div></>
        <div className="col-md-12 pt-4 p-0 hive_profile_navigation_divider d-lg-none" id=""></div>
        {/*    Navigation isle      */}
        <div className="row justify-content-center m-0 p-0 col-md-12" id="">
          {/*    Image section isle      */}
          
          {/*    Image section isle      */}
          
          {/*  //-------------    main content starts here  ------------------------------ */}
          
          
          
          <div className="col-md-12 row justify-content-center m-0  p-0">
            {/*    Input cells section isle      */}
            <div className="col-md-12 row p-0 justify-content-center p-0 m-0">
              <div className="col-md-12 bg-white border border_set shadow-md p-4 mb-4 hive_form_section  ">
                <h5 className="col-md-12 row p-2 justify-content-center p-0 m-0">
                  <div className="col-md-3 bg-dark mb-3 mb-lg-0 mt-lg-3" style={{height: "1px"}}></div>
                  <div className="col-md-5 text-center">Basic Information</div>
                  <div className="col-md-4 bg-dark mt-3" style={{height: "1px"}}></div>
                </h5>
                
                <div className="col-md-12 pt-3 p-0" id=""></div>
                
                <div className="row justify-content-start col-md-12 p-0 m-0 ">
                  <LiveSearchDropdown
                  apiEndpoint={apiRoutes.systemusers.base}
                  tblName="system_users"
                  parentTable="user_manifest_"
                  inputName="_system_users_name_user_id"
                  hiddenInputName="user_id"
                  valueField="record_id"
                  displayField="name"
                  label="Name"
                  defaultValue={{ record_id: user_manifest_Node?.user_id || "", name: user_manifest_Node?._system_users_name_user_id || "" }}
                  onSelect={(id) => console.log("Just the ID:", id)}
                  onSelectFull={(dataRes) =>  console.log("Data seleted")}
                  onInputChange={handleInputChange}
                  defaultColSize={`col-md-4 hive_data_cell  hive_data_cell ${
                    customProfileData?.user_id
                    ? 'd-none'
                    : ''
                  }`}
                  context={{hostParent : hostParent}}
                  />
                  
                  <MosySmartField
                  module="user_manifest_"
                  field="user_name"
                  label="User Name"
                  value={user_manifest_Node?.user_name || ""}
                  onChange={handleInputChange}
                  context={{ hostParent: hostParent  }}
                  inputOverrides={{}}
                  type="text"
                  cellOverrides={{additionalClass: "col-md-4 hive_data_cell "}}
                  />
                  
                  
                  <MosySmartField
                  module="user_manifest_"
                  field="role_id"
                  label="Role Id"
                  value={user_manifest_Node?.role_id || ""}
                  onChange={handleInputChange}
                  context={{ hostParent: hostParent  }}
                  inputOverrides={{}}
                  type="text"
                  cellOverrides={{additionalClass: "col-md-4 hive_data_cell "}}
                  />
                  
                  
                  <MosySmartField
                  module="user_manifest_"
                  field="site_id"
                  label="Site Id"
                  value={user_manifest_Node?.site_id || ""}
                  onChange={handleInputChange}
                  context={{ hostParent: hostParent  }}
                  inputOverrides={{}}
                  type="text"
                  cellOverrides={{additionalClass: "col-md-4 hive_data_cell "}}
                  />
                  
                  
                  <MosySmartField
                  module="user_manifest_"
                  field="role_name"
                  label="Role Name"
                  value={user_manifest_Node?.role_name || ""}
                  onChange={handleInputChange}
                  context={{ hostParent: hostParent  }}
                  inputOverrides={{}}
                  type="text"
                  cellOverrides={{additionalClass: "col-md-4 hive_data_cell "}}
                  />
                  
                </div>
                
              </div>
              
              <div className="col-md-12 bg-white border border_set shadow-md p-4 mb-4 hive_form_section  ">
                <h5 className="col-md-12 row p-2 justify-content-center p-0 m-0">
                  <div className="col-md-3 bg-dark mb-3 mb-lg-0 mt-lg-3" style={{height: "1px"}}></div>
                  <div className="col-md-5 text-center"></div>
                  <div className="col-md-4 bg-dark mt-3" style={{height: "1px"}}></div>
                </h5>
                
                <div className="col-md-12 pt-3 p-0" id=""></div>
                
                <div className="row justify-content-start col-md-12 p-0 m-0 ">
                  
                  <MosySmartField
                  module="user_manifest_"
                  field="project_id"
                  label="Project Id"
                  value={user_manifest_Node?.project_id || ""}
                  onChange={handleInputChange}
                  context={{ hostParent: hostParent  }}
                  inputOverrides={{}}
                  type="text"
                  cellOverrides={{additionalClass: "col-md-4 hive_data_cell "}}
                  />
                  
                  
                  <MosySmartField
                  module="user_manifest_"
                  field="project_name"
                  label="Project Name"
                  value={user_manifest_Node?.project_name || ""}
                  onChange={handleInputChange}
                  context={{ hostParent: hostParent  }}
                  inputOverrides={{}}
                  type="text"
                  cellOverrides={{additionalClass: "col-md-4 hive_data_cell "}}
                  />
                  
                </div>
                
                <div className="col-md-12 text-center">
                  <SubmitButtons
                  src="UserManifestMainProfilePage"
                  tblName="user_manifest_"
                  extraClass="optional-custom-class"
                  
                  />
                </div>
              </div></div>
              {/*    Input cells section isle      */}
            </div>
            
            <section className="hive_control">
              <input type="hidden" id="user_manifest__dataNode" name="user_manifest__dataNode" value={paramUserManifestUptoken}/>
              <input type="hidden" id="user_manifest__mosy_action" name="user_manifest__mosy_action" value={userManifestActionStatus}/>
            </section>
            
            
          </div>
          
        </form>
        
        
        <div className="row justify-content-center m-0 pr-lg-1 pl-lg-1 pt-0 col-md-12" id="">
          {/*<hive_mini_list/>*/}
          
          {user_manifest_Node?.primkey && (
            <MosyProfileSection
            title={`User Details`}
            source="user_manifest__SystemUsersProfile"
            component={SystemUsersProfile}
            table="user_manifest_"
            key={`SystemUsersProfile-${localEventSignature}`}
            dataIn={{
              
              parentStateSetters : stateItemSetters,
              parentUseEffectKey : localEventSignature,
              showNavigationIsle:false,
              customQueryStr : systemUsersCustomProfileQuery,
              hostParent : "UserManifestProfile",
              parentProfileItemId : activeScrollId,
              customProfileData : {
                _user_manifest__user_name_record_id:user_manifest_Node?.user_name,
                record_id:user_manifest_Node?.user_id
              }
              
            }}
            
            dataOut={{
              
              setChildDataOut: InteprateSystemUsersEvent,
              setChildDataOutSignature: (sig) => console.log("Signature changed:", sig),
              
            }}
            />
            
          )}
          
          
          {user_manifest_Node?.primkey && (
            <MosyProfileSection
            viewAllLink={`../systemusers/list?user_manifest__mosyfilter=${btoa(`{recordId:btoa(user_manifest_Node?.user_id)    }`)}`}
            title={`User Details`}
            source="user_manifest__SystemUsersList"
            component={SystemUsersList}
            table="user_manifest_"
            key={`SystemUsersList-${localEventSignature}`}
            dataIn={{
              parentStateSetters : stateItemSetters,
              parentUseEffectKey : localEventSignature,
              showNavigationIsle:false,
              showDataControlSections:false,
              customQueryStr : {recordId:btoa(user_manifest_Node?.user_id)    },
              customProfilePath:"../systemusers/profile"
              
            }}
            
            dataOut={{
              setChildDataOut: InteprateSystemUsersEvent,
              setChildDataOutSignature: (sig) => console.log("Signature changed:", sig),
            }}
            />
          )}
        </div>
      </div>
    </div>
    
    
    {/* snack notifications -- */}
    {snackMessage &&(
      <MosySnackWidget
      content={snackMessage}
      duration={5000}
      type="custom"
      onDone={() => {
        stateItemSetters.setSnackMessage("");
        stateItem.snackOnDone(); // Run whats inside onDone
        deleteUrlParam("snack_alert")
      }}
      
      />)}
      {/* snack notifications -- */}
      
      
      {/* ================== End Feature Section========================== ------*/}
    </div>
    
  );
  
}

