...
const PhonebookActions = {
  ...
  // Opens the shared call launcher (phone/WhatsApp). Phonebook's own
  // field names (name/phone/email) already sit inside the generic
  // fallback chains MosyCommsSmartCall reads (full_name/business_name/
  // NAME, phone_number/tel/PHONE) — no mapping needed, raw row passes
  // straight through, same as clients/leads.
  call: ({ rows }) => {
    const row = rows?.[0];
    if (!row) return;
  ...
  // Opens the shared smart-message composer — same reasoning as call:
  // name/phone/email already match MosySendSmartMessage's generic
  // fallback keys, so the raw row works unmapped.
  send_message: ({ rows }) => {
    const row = rows?.[0];
    if (!row) return;
