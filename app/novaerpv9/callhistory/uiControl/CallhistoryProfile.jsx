...
// CallhistoryProfile — pure shell. It resolves the id, wires up the
...
export default function CallhistoryProfile({ id: idProp, onDone, hiddenActions = [], presetValues, schemaOverride }) {
  const searchParams = useSearchParams();
  const schema = schemaOverride || CallhistorySchema;
  const id = idProp ?? searchParams.get(`${CallhistorySchema.entity}_dataNode`);
  const form = useEntityFormController(schema, CallhistoryActions, {
    id,
    onDone,
