...
// QuotationsProfile — pure shell. It resolves the id, wires up the
...
export default function QuotationsProfile({ id: idProp, onDone, hiddenActions = [], presetValues, schemaOverride }) {
  const searchParams = useSearchParams();
  const schema = schemaOverride || QuotationsSchema;
  const id = idProp ?? searchParams.get(`${QuotationsSchema.entity}_dataNode`);
  const form = useEntityFormController(schema, QuotationsActions, {
    id,
    onDone,
