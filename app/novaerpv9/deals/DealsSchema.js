...
const moduleApi = apiRoutes.deals.base;

export const DealsSchema = {
  entity: 'deals',              // DB table name; also drives default role names
                                     // (view_deals / manage_deals) and apiBase
  label: 'Deals',                 // optional, defaults to entity capitalized
  apiBase: moduleApi,

  //api endpint for importing data from csv
  importDataEndpoint : apiRoutes.deals.import,
  ...
  profileActions: [
    { key: 'back', label: 'Back to list', icon: 'arrow-left', variant: 'outline-secondary', navigateTo: '/novaerpv9/deals/list', grid: false, form: true },
    { key: 'save', label: 'Save', icon: 'save', variant: 'primary', grid: false, form: true, rowAction: false },
    { key: 'delete', label: 'Delete', icon: 'trash', variant: 'outline-danger', confirm: 'Are you sure you want to delete this deal?', editOnly: true, grid: false, form: true, rowAction: true, role: 'manage_deals' },
    { key: 'view', label: 'View more', icon: 'edit', rowAction: true },
    { key: 'new', label: 'New Deal', icon: 'plus',  colorClass: 'dyn-btn-accent-green', navigateTo: '/novaerpv9/deals/profile', grid: true, form: false, rowAction: false },
    { key: 'clone', label: 'Clone Record', icon: 'copy', variant: 'outline-secondary', editOnly: true, grid: false, form: true, rowAction: false, role: 'manage_deals' },
    ...
    { key: 'filter_by_deal_status', label: 'Filter by status', icon: 'filter', colorClass: 'dyn-btn-accent-yellow', type: 'action', grid: true, form: false, rowAction: false },
    { key: 'filter_by_deal_stage', label: 'Filter by stage', icon: 'filter', colorClass: 'dyn-btn-accent-blue', type: 'action', grid: true, form: false, rowAction: false },
  ...
  showInList: ['row_count', 'deal_title', 'deal_description', 'client_id', 'deal_source', 'deal_value', 'expected_close_date', 'pipeline_stage'],

  //export columns these columns are used to generate upload csv template file
  exportColumns: ['deal_title', 'deal_description', 'client_id', 'deal_source', 'deal_value', 'expected_close_date', 'pipeline_stage'],

  sections: [
    { key: 'basic_information', label: 'Basic Information', columns: 3, fields: ['deal_title', 'deal_description', 'client_id', 'deal_source', 'deal_value', 'expected_close_date', 'pipeline_stage'] },
    { key: 'other_details', label: 'Other Details', columns: 3, fields: ['deal_stage', 'deal_status', 'assigned_sales_rep', 'priority_level', 'deal_probability', 'next_follow_up_date', 'deal_notes'] },
  ],
  ...
  fields: [
    ...
    { key: 'deal_probability', label: 'Deal Probability', type: 'text' },
    { key: 'next_follow_up_date', label: 'Next Follow Up Date', type: 'datetime' },
    { key: 'deal_notes', label: 'Deal Notes', type: 'textarea', colSpan: 3 },
...
// dyn-btn-accent-teal   { border-color: transparent; color: #0f766e; }
  ...
  // roles: { view: 'view_deals', manage: 'manage_deals' },
};
