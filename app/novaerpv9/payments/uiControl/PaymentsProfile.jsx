...
// PaymentsProfile — pure shell. It resolves the id, wires up the
...
export default function PaymentsProfile({ id: idProp, onDone, hiddenActions = [], presetValues, schemaOverride }) {
  const searchParams = useSearchParams();
  const schema = schemaOverride || PaymentsSchema;
  const id = idProp ?? searchParams.get(`${PaymentsSchema.entity}_dataNode`);
  const form = useEntityFormController(schema, PaymentsActions, {
    id,
    onDone,
