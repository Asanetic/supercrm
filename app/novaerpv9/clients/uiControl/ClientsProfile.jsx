...
export default function ClientsProfile({ id: idProp, onDone, hiddenActions = [], presetValues, schemaOverride }) {
  const searchParams = useSearchParams();
  const schema = schemaOverride || ClientsSchema;
  const id = idProp ?? searchParams.get(`${ClientsSchema.entity}_dataNode`);
  const form = useEntityFormController(schema, ClientsActions, {
    id,
    onDone,
