...
export default function LeadsProfile({ id: idProp, onDone, hiddenActions = [], presetValues, schemaOverride }) {
  const searchParams = useSearchParams();
  const schema = schemaOverride || LeadsSchema;
  const id = idProp ?? searchParams.get(`${LeadsSchema.entity}_dataNode`);
  const form = useEntityFormController(schema, LeadsActions, {
    id,
    onDone,
