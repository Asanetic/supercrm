...
const moduleApi = apiRoutes.products.base;

export const ProductsSchema = {
  entity: 'products',              // DB table name; also drives default role names
                                     // (view_products / manage_products) and apiBase
  label: 'Products',                 // optional, defaults to entity capitalized
  apiBase: moduleApi,

  //api endpint for importing data from csv
  importDataEndpoint : apiRoutes.products.import,
  ...
  profileActions: [
    { key: 'back', label: 'Back to list', icon: 'arrow-left', variant: 'outline-secondary', navigateTo: '/supererpv7/products/list', grid: false, form: true },
    { key: 'save', label: 'Save', icon: 'save', variant: 'primary', grid: false, form: true, rowAction: false },
    { key: 'delete', label: 'Delete', icon: 'trash', variant: 'outline-danger', confirm: 'Are you sure you want to delete this product?', editOnly: true, grid: false, form: true, rowAction: true, role: 'manage_products' },
    { key: 'view', label: 'View more', icon: 'edit', rowAction: true },
    { key: 'new', label: 'New Product', icon: 'plus', variant: 'outline-primary', navigateTo: '/supererpv7/products/profile', grid: true, form: false, rowAction: false },
    { key: 'clone', label: 'Clone Record', icon: 'copy', variant: 'outline-secondary', editOnly: true, grid: false, form: true, rowAction: false, role: 'manage_products' },
  ...
  showInList: ['row_count',  'product_name', 'price_range', 'unit_price', 'product_code', 'category', 'product_description'],

  //export columns these columns are used to generate upload csv template file
  exportColumns: ['product_name', 'price_range', 'unit_price', 'product_code', 'category', 'product_description', 'product_image'],

  sections: [
    { key: 'basic_information', label: 'Basic Information', columns: 3, fields: ['product_image', 'product_name', 'price_range', 'unit_price', 'product_code', 'category', 'product_description'] },
    { key: 'other_details', label: 'Other Details', columns: 3, fields: ['discount_price', 'tax_percentage', 'currency_code', 'stock_quantity', 'product_status'] },
  ],
  ...
  fields: [
    ...
    { key: 'product_name', label: 'Product Name', type: 'text', title: true },
    { key: 'price_range', label: 'Price Range', type: 'text' },
    { key: 'unit_price', label: 'Unit Price', type: 'money' , sum:true},
    { key: 'product_code', label: 'Product Code', type: 'text', title: true },
    { key: 'category', label: 'Category', type: 'groupedSelect', endpoint: moduleApi, groupByField: 'category' },
    { key: 'product_description', label: 'Product Description', type: 'textarea', colSpan: 12 },
    { key: 'product_image', label: 'Product Image', type: 'image', colSpan: 12 },
    { key: 'discount_price', label: 'Discount Price', type: 'money' },
    { key: 'tax_percentage', label: 'Tax Percentage', type: 'text' },
    { key: 'currency_code', label: 'Currency Code', type: 'text', title: true },
    { key: 'stock_quantity', label: 'Stock Quantity', type: 'text' },
    { key: 'product_status', label: 'Product Status', type: 'text' },
