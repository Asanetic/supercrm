...
// ProductsProfile — pure shell. It resolves the id, wires up the
...
export default function ProductsProfile({ id: idProp, onDone, hiddenActions=[] }) {
  const searchParams = useSearchParams();
  const id = idProp ?? searchParams.get(`${ProductsSchema.entity}_dataNode`);
  const form = useEntityFormController(ProductsSchema, ProductsActions, {
    id,
    onDone,
  ...
  return (
    ...
    <DynamicForm
      controller={form}
      eyebrow={form.isEditing ? 'Products Profile' : 'Products Directory'}
      title={form.isEditing ? mosyGetSchemaTitle(ProductsSchema, form.values, '') : 'New Products'}
