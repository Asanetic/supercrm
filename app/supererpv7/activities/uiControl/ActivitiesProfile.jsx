...
// ActivitiesProfile — pure shell. It resolves the id, wires up the
...
export default function ActivitiesProfile({ id: idProp, onDone, hiddenActions=[] }) {
  const searchParams = useSearchParams();
  const id = idProp ?? searchParams.get(`${ActivitiesSchema.entity}_dataNode`);
  const form = useEntityFormController(ActivitiesSchema, ActivitiesActions, {
    id,
    onDone,
  ...
  return (
    ...
    <DynamicForm
      controller={form}
      eyebrow={form.isEditing ? 'Activities Profile' : 'Activities Directory'}
      title={form.isEditing ? mosyGetSchemaTitle(ActivitiesSchema, form.values, '') : 'New Activities'}
