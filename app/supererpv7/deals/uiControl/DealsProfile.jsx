...
export default function DealsProfile({ id: idProp, onDone, hiddenActions=[] }) {
  const searchParams = useSearchParams();
  const id = idProp ?? searchParams.get(`${DealsSchema.entity}_dataNode`);
  const form = useEntityFormController(DealsSchema, DealsActions, {
    id,
    onDone,
  ...
  return (
    ...
    <DynamicForm
      controller={form}
      eyebrow={form.isEditing ? 'Deals Profile' : 'Deals Directory'}
      title={form.isEditing ? mosyGetSchemaTitle(DealsSchema, form.values, '') : 'New Deals'}
