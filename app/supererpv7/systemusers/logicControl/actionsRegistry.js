...
import { reviewUserAcc } from "./systemuser-actions";
...
const SystemusersActions = {
  ...
  review_account: async ({ schema, create, refresh, rows }) =>reviewUserAcc(rows, schema,"Disable account", "Update roles"), // now returns the contract itself

  view_role_permissions: async ({ schema, create, refresh, rows }) => {
    const row = rows?.[0];
    console.log(`view_role_permissions`, row)
    MosyCard("", <div className="row justify-content-center col-md-12 p-0 m-0 "><RolePermissionMatrix roleId={row?.user_role}/></div>, false, "modal2","mosycard_wide");
  },
...
export default SystemusersActions
