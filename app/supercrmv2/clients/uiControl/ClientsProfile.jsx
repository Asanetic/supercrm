...
// ════════════════════════════════════════════════════════════════
// Imports from send-client-message.jsx
import {
  sendClientMessage
} from '../logicControl/send-client-message';
...
export default function ClientsProfile({ dataIn = {}, dataOut = {} }) {
  ...
  useEffect(() => {
    if (clientsNode?.primkey && setLeadsCustomProfileQuery) {
      
      const query = {recordId:btoa(clientsNode?.converted_lead_id)                                };
  ...
  }, [clientsNode, setLeadsCustomProfileQuery]);
  ...
  useEffect(() => {
    if (clientsNode?.primkey && setSmartMessagesCustomProfileQuery) {
      
      const query = {relatedRecordId:btoa(clientsNode?.record_id)                                    };
  ...
  }, [clientsNode, setSmartMessagesCustomProfileQuery]);
      ...
      <div className="col-md-12 row p-0 justify-content-center p-0 m-0">
        <div className="col-md-12 bg-white border border_set shadow-md p-4 mb-4 hive_form_section  ">
          ...
          <div className="row justify-content-start col-md-12 p-0 m-0 ">
            ...
            tblName="leads"
            parentTable="clients"
            inputName="_leads_full_name_converted_lead_id"
            hiddenInputName="converted_lead_id"
            valueField="record_id"
            displayField="full_name"
            label="Converted lead"
            defaultValue={{ record_id: clientsNode?.converted_lead_id || "", full_name: clientsNode?._leads_full_name_converted_lead_id || "" }}
            onSelect={(id) => console.log("Just the ID:", id)}
            onSelectFull={(dataRes) =>  console.log("Data seleted")}
            onInputChange={handleInputChange}
            defaultColSize={`col-md-4 hive_data_cell  hive_data_cell ${
              customProfileData?.converted_lead_id
              ? 'd-none'
              : ''
            }`}
            context={{hostParent : hostParent}}
  ...
  <div className="row justify-content-center m-0 pr-lg-1 pl-lg-1 pt-0 col-md-12" id="">
    ...
    {clientsNode?.primkey && (
      ...
      source="clients_SmartMessagesProfile"
    ...
    {clientsNode?.primkey && (
      <MosyProfileSection
      viewAllLink={`../leads/list?clients_mosyfilter=${btoa(`{recordId:btoa(clientsNode?.converted_lead_id)                                }`)}`}
      title={`Lead profile`}
      source="clients_LeadsList"
      component={LeadsList}
      table="clients"
      ...
      dataIn={{
        ...
        showNavigationIsle:false,
        showDataControlSections:false,
        customQueryStr : {recordId:btoa(clientsNode?.converted_lead_id)                                },
    ...
    {clientsNode?.primkey && (
      <MosyProfileSection
      viewAllLink={`../smartmessages/list?clients_mosyfilter=${btoa(`{relatedRecordId:btoa(clientsNode?.record_id)                                    }`)}`}
      title={`Message history`}
      source="clients_SmartMessagesList"
      ...
      dataIn={{
        ...
        showNavigationIsle:false,
        showDataControlSections:false,
        customQueryStr : {relatedRecordId:btoa(clientsNode?.record_id)                                    },
