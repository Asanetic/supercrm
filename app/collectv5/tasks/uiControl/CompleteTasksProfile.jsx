...
// CompleteTasksProfile — pure shell. It resolves the id, wires up the
...
export default function CompleteTasksProfile({ id: idProp, onDone, hiddenActions=[] }) {
  const searchParams = useSearchParams();
  const id = idProp ?? searchParams.get(`${CompleteTasksSchema.entity}_dataNode`);
  const form = useEntityFormController(CompleteTasksSchema, TasksActions, {
    id,
    onDone,
  ...
  return (
    <DynamicForm
      controller={form}
      eyebrow={form.isEditing ? `${CompleteTasksSchema.label}  Profile` : `${CompleteTasksSchema.label}  Directory`}
      title={form.isEditing ? mosyGetSchemaTitle(CompleteTasksSchema, form.values, '') : `New ${CompleteTasksSchema.label}`}
