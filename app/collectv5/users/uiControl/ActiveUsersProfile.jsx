...
// ActiveUsersProfile — pure shell. It resolves the id, wires up the
...
export default function ActiveUsersProfile({ id: idProp, onDone, hiddenActions=[] }) {
  const searchParams = useSearchParams();
  const id = idProp ?? searchParams.get(`${ActiveUsersSchema.entity}_dataNode`);
  const form = useEntityFormController(ActiveUsersSchema, UsersActions, {
    id,
    onDone,
  ...
  return (
    <DynamicForm
      controller={form}
      eyebrow={form.isEditing ? `${ActiveUsersSchema.label}  Profile` : `${ActiveUsersSchema.label}  Directory`}
      title={form.isEditing ? mosyGetSchemaTitle(ActiveUsersSchema, form.values, '') : `New ${ActiveUsersSchema.label}`}
