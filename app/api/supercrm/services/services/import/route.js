...
import { AddServices } from "../ServicesDbGateway";
...
export async function POST(req) {
  try {
    ...
    for (const node of processedInsertObj) {
      ...
      const mutatedObj = mutateInputArray("services", node, req, newRecordId, authData);

     const insertResult = await AddServices(newRecordId, mutatedObj, req, authData);
