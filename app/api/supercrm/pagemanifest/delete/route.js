...
import { DeletePageManifest } from '../pagemanifest/PageManifestDbGateway';
...
export async function GET(request) {
  ...
  const deleteToken = searchParams.get('_page_manifest__delete_record');
  const deleteTokenDecode = base64Decode(deleteToken);

  if (deleteToken) {
    // Customize table and WHERE clause here
    const table = 'page_manifest_'; // Replace with your actual table
    
     
    
    const whereStr = `WHERE primkey = '${deleteTokenDecode}'`;

    const res = await DeletePageManifest(deleteTokenDecode, whereStr);
