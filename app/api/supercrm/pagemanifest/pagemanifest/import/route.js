...
import { AddPageManifest } from "../PageManifestDbGateway";
...
export async function POST(req) {
  try {
    ...
    for (const node of processedInsertObj) {
      ...
      node.manikey = newRecordId;
      
      const mutatedObj = mutateInputArray("page_manifest_", node, req, newRecordId, authData);

     const insertResult = await AddPageManifest(newRecordId, mutatedObj, req, authData);
