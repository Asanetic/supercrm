...
import { AddUserManifest } from "../UserManifestDbGateway";
...
export async function POST(req) {
  try {
    ...
    for (const node of processedInsertObj) {
      ...
      node.admin_mkey = newRecordId;
      
      const mutatedObj = mutateInputArray("user_manifest_", node, req, newRecordId, authData);

     const insertResult = await AddUserManifest(newRecordId, mutatedObj, req, authData);
