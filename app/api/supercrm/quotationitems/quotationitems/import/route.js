...
import { AddQuotationItems } from "../QuotationItemsDbGateway";
...
export async function POST(req) {
  try {
    ...
    for (const node of processedInsertObj) {
      ...
      const mutatedObj = mutateInputArray("quotation_items", node, req, newRecordId, authData);

     const insertResult = await AddQuotationItems(newRecordId, mutatedObj, req, authData);
