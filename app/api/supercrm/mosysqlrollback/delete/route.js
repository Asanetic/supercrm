...
import { DeleteMosySqlRollBack } from '../mosysqlrollback/MosySqlRollBackDbGateway';
...
export async function GET(request) {
  ...
  const deleteToken = searchParams.get('_mosy_sql_roll_back_delete_record');
  const deleteTokenDecode = base64Decode(deleteToken);

  if (deleteToken) {
    // Customize table and WHERE clause here
    const table = 'mosy_sql_roll_back'; // Replace with your actual table
    
     
    
    const whereStr = `WHERE primkey = '${deleteTokenDecode}'`;

    const res = await DeleteMosySqlRollBack(deleteTokenDecode, whereStr);
