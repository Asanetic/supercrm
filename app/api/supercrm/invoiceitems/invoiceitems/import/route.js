...
import { AddInvoiceItems } from "../InvoiceItemsDbGateway";
...
export async function POST(req) {
  try {
    ...
    for (const node of processedInsertObj) {
      ...
      const mutatedObj = mutateInputArray("invoice_items", node, req, newRecordId, authData);

     const insertResult = await AddInvoiceItems(newRecordId, mutatedObj, req, authData);
