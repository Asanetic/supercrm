...
import { DeleteTasks } from '../tasks/TasksDbGateway';
...
export async function GET(request) {
  ...
  const deleteToken = searchParams.get('_tasks_delete_record');
  const deleteTokenDecode = base64Decode(deleteToken);

  if (deleteToken) {
    // Customize table and WHERE clause here
    const table = 'tasks'; // Replace with your actual table
    
     
    
    const whereStr = `WHERE primkey = '${deleteTokenDecode}'`;

    const res = await DeleteTasks(deleteTokenDecode, whereStr);
