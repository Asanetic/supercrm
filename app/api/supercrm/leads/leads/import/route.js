...
import { AddLeads } from "../LeadsDbGateway";
...
export async function POST(req) {
  try {
    ...
    for (const node of processedInsertObj) {
      ...
      const mutatedObj = mutateInputArray("leads", node, req, newRecordId, authData);

     const insertResult = await AddLeads(newRecordId, mutatedObj, req, authData);
