...
import { AddDeals } from "../DealsDbGateway";
...
export async function POST(req) {
  try {
    ...
    for (const node of processedInsertObj) {
      ...
      const mutatedObj = mutateInputArray("deals", node, req, newRecordId, authData);

     const insertResult = await AddDeals(newRecordId, mutatedObj, req, authData);
