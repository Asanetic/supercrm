...
import { DeleteExpectedRevenue } from '../expectedrevenue/ExpectedRevenueDbGateway';
...
export async function GET(request) {
  ...
  const deleteToken = searchParams.get('_expected_revenue_delete_record');
  const deleteTokenDecode = base64Decode(deleteToken);

  if (deleteToken) {
    // Customize table and WHERE clause here
    const table = 'expected_revenue'; // Replace with your actual table
    
     
    
    const whereStr = `WHERE primkey = '${deleteTokenDecode}'`;

    const res = await DeleteExpectedRevenue(deleteTokenDecode, whereStr);
