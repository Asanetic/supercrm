...
import { AddProductCategories } from "../ProductCategoriesDbGateway";
...
export async function POST(req) {
  try {
    ...
    for (const node of processedInsertObj) {
      ...
      const mutatedObj = mutateInputArray("product_categories", node, req, newRecordId, authData);

     const insertResult = await AddProductCategories(newRecordId, mutatedObj, req, authData);
