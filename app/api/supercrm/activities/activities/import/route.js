...
import { AddActivities } from "../ActivitiesDbGateway";
...
export async function POST(req) {
  try {
    ...
    for (const node of processedInsertObj) {
      ...
      const mutatedObj = mutateInputArray("activities", node, req, newRecordId, authData);

     const insertResult = await AddActivities(newRecordId, mutatedObj, req, authData);
