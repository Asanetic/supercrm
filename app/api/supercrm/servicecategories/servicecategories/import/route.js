...
import { AddServiceCategories } from "../ServiceCategoriesDbGateway";
...
export async function POST(req) {
  try {
    ...
    for (const node of processedInsertObj) {
      ...
      const mutatedObj = mutateInputArray("service_categories", node, req, newRecordId, authData);

     const insertResult = await AddServiceCategories(newRecordId, mutatedObj, req, authData);
