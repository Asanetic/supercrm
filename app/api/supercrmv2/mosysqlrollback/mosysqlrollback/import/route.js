...
import { AddMosySqlRollBack } from "../MosySqlRollBackDbGateway";
...
export async function POST(req) {
  try {
    ...
    for (const node of processedInsertObj) {
      ...
      node.roll_bk_key = newRecordId;
      
      const mutatedObj = mutateInputArray("mosy_sql_roll_back", node, req, newRecordId, authData);

     const insertResult = await AddMosySqlRollBack(newRecordId, mutatedObj, req, authData);
