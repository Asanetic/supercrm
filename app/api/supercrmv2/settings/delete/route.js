...
import { DeleteSettings } from '../settings/SettingsDbGateway';
...
export async function GET(request) {
  ...
  const deleteToken = searchParams.get('_settings_delete_record');
  const deleteTokenDecode = base64Decode(deleteToken);

  if (deleteToken) {
    // Customize table and WHERE clause here
    const table = 'settings'; // Replace with your actual table
    
     
    
    const whereStr = `WHERE primkey = '${deleteTokenDecode}'`;

    const res = await DeleteSettings(deleteTokenDecode, whereStr);
