...
import { AddSettings } from "../SettingsDbGateway";
...
export async function POST(req) {
  try {
    ...
    for (const node of processedInsertObj) {
      ...
      const mutatedObj = mutateInputArray("settings", node, req, newRecordId, authData);

     const insertResult = await AddSettings(newRecordId, mutatedObj, req, authData);
