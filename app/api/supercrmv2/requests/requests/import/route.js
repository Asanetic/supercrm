...
import { AddRequests } from "../RequestsDbGateway";
...
export async function POST(req) {
  try {
    ...
    for (const node of processedInsertObj) {
      ...
      const mutatedObj = mutateInputArray("requests", node, req, newRecordId, authData);

     const insertResult = await AddRequests(newRecordId, mutatedObj, req, authData);
