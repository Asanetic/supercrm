...
import { AddSmartPaymentRequests } from "../SmartPaymentRequestsDbGateway";
...
export async function POST(req) {
  try {
    ...
    for (const node of processedInsertObj) {
      ...
      const mutatedObj = mutateInputArray("smart_payment_requests", node, req, newRecordId, authData);

     const insertResult = await AddSmartPaymentRequests(newRecordId, mutatedObj, req, authData);
