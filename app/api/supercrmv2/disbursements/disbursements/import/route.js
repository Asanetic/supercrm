...
import { AddDisbursements } from "../DisbursementsDbGateway";
...
export async function POST(req) {
  try {
    ...
    for (const node of processedInsertObj) {
      ...
      const mutatedObj = mutateInputArray("disbursements", node, req, newRecordId, authData);

     const insertResult = await AddDisbursements(newRecordId, mutatedObj, req, authData);
