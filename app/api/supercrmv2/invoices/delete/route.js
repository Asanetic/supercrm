...
import { DeleteInvoices } from '../invoices/InvoicesDbGateway';
...
export async function GET(request) {
  ...
  const deleteToken = searchParams.get('_invoices_delete_record');
  const deleteTokenDecode = base64Decode(deleteToken);

  if (deleteToken) {
    // Customize table and WHERE clause here
    const table = 'invoices'; // Replace with your actual table
    
     
    
    const whereStr = `WHERE primkey = '${deleteTokenDecode}'`;

    const res = await DeleteInvoices(deleteTokenDecode, whereStr);
