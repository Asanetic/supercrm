...
import { DeleteQuotationItems } from '../quotationitems/QuotationItemsDbGateway';
...
export async function GET(request) {
  ...
  const deleteToken = searchParams.get('_quotation_items_delete_record');
  const deleteTokenDecode = base64Decode(deleteToken);

  if (deleteToken) {
    // Customize table and WHERE clause here
    const table = 'quotation_items'; // Replace with your actual table
    
     
    
    const whereStr = `WHERE primkey = '${deleteTokenDecode}'`;

    const res = await DeleteQuotationItems(deleteTokenDecode, whereStr);
