...
import { AddSystemUsers } from "../SystemUsersDbGateway";
...
export async function POST(req) {
  try {
    ...
    for (const node of processedInsertObj) {
      ...
      const mutatedObj = mutateInputArray("system_users", node, req, newRecordId, authData);

     const insertResult = await AddSystemUsers(newRecordId, mutatedObj, req, authData);
