...
import { DeleteSmartPayments } from '../smartpayments/SmartPaymentsDbGateway';
...
export async function GET(request) {
  ...
  const deleteToken = searchParams.get('_smart_payments_delete_record');
  const deleteTokenDecode = base64Decode(deleteToken);

  if (deleteToken) {
    // Customize table and WHERE clause here
    const table = 'smart_payments'; // Replace with your actual table
    
     
    
    const whereStr = `WHERE primkey = '${deleteTokenDecode}'`;

    const res = await DeleteSmartPayments(deleteTokenDecode, whereStr);
