...
import { AddSmartPayments } from "../SmartPaymentsDbGateway";
...
export async function POST(req) {
  try {
    ...
    for (const node of processedInsertObj) {
      ...
      const mutatedObj = mutateInputArray("smart_payments", node, req, newRecordId, authData);

     const insertResult = await AddSmartPayments(newRecordId, mutatedObj, req, authData);
