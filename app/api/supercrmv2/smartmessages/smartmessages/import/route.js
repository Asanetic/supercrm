...
import { AddSmartMessages } from "../SmartMessagesDbGateway";
...
export async function POST(req) {
  try {
    ...
    for (const node of processedInsertObj) {
      ...
      const mutatedObj = mutateInputArray("smart_messages", node, req, newRecordId, authData);

     const insertResult = await AddSmartMessages(newRecordId, mutatedObj, req, authData);
