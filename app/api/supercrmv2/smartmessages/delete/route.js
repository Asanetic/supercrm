...
import { DeleteSmartMessages } from '../smartmessages/SmartMessagesDbGateway';
...
export async function GET(request) {
  ...
  const deleteToken = searchParams.get('_smart_messages_delete_record');
  const deleteTokenDecode = base64Decode(deleteToken);

  if (deleteToken) {
    // Customize table and WHERE clause here
    const table = 'smart_messages'; // Replace with your actual table
    
     
    
    const whereStr = `WHERE primkey = '${deleteTokenDecode}'`;

    const res = await DeleteSmartMessages(deleteTokenDecode, whereStr);
