...
import { AddExpectedRevenue } from "../ExpectedRevenueDbGateway";
...
export async function POST(req) {
  try {
    ...
    for (const node of processedInsertObj) {
      ...
      const mutatedObj = mutateInputArray("expected_revenue", node, req, newRecordId, authData);

     const insertResult = await AddExpectedRevenue(newRecordId, mutatedObj, req, authData);
