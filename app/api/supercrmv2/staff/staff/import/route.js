...
import { AddStaff } from "../StaffDbGateway";
...
export async function POST(req) {
  try {
    ...
    for (const node of processedInsertObj) {
      ...
      const mutatedObj = mutateInputArray("staff", node, req, newRecordId, authData);

     const insertResult = await AddStaff(newRecordId, mutatedObj, req, authData);
