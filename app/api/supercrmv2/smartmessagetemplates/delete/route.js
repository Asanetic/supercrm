...
import { DeleteSmartMessageTemplates } from '../smartmessagetemplates/SmartMessageTemplatesDbGateway';
...
export async function GET(request) {
  ...
  const deleteToken = searchParams.get('_smart_message_templates_delete_record');
  const deleteTokenDecode = base64Decode(deleteToken);

  if (deleteToken) {
    // Customize table and WHERE clause here
    const table = 'smart_message_templates'; // Replace with your actual table
    
     
    
    const whereStr = `WHERE primkey = '${deleteTokenDecode}'`;

    const res = await DeleteSmartMessageTemplates(deleteTokenDecode, whereStr);
