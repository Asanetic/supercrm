...
import { AddSmartMessageTemplates } from "../SmartMessageTemplatesDbGateway";
...
export async function POST(req) {
  try {
    ...
    for (const node of processedInsertObj) {
      ...
      const mutatedObj = mutateInputArray("smart_message_templates", node, req, newRecordId, authData);

     const insertResult = await AddSmartMessageTemplates(newRecordId, mutatedObj, req, authData);
