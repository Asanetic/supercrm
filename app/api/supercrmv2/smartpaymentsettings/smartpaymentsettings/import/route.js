...
import { AddSmartPaymentSettings } from "../SmartPaymentSettingsDbGateway";
...
export async function POST(req) {
  try {
    ...
    for (const node of processedInsertObj) {
      ...
      const mutatedObj = mutateInputArray("smart_payment_settings", node, req, newRecordId, authData);

     const insertResult = await AddSmartPaymentSettings(newRecordId, mutatedObj, req, authData);
