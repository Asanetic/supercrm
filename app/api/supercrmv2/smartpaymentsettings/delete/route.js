...
import { DeleteSmartPaymentSettings } from '../smartpaymentsettings/SmartPaymentSettingsDbGateway';
...
export async function GET(request) {
  ...
  const deleteToken = searchParams.get('_smart_payment_settings_delete_record');
  const deleteTokenDecode = base64Decode(deleteToken);

  if (deleteToken) {
    // Customize table and WHERE clause here
    const table = 'smart_payment_settings'; // Replace with your actual table
    
     
    
    const whereStr = `WHERE primkey = '${deleteTokenDecode}'`;

    const res = await DeleteSmartPaymentSettings(deleteTokenDecode, whereStr);
