...
import { AddUserBundleRoleFunctions } from "../UserBundleRoleFunctionsDbGateway";
...
export async function POST(req) {
  try {
    ...
    for (const node of processedInsertObj) {
      ...
      const mutatedObj = mutateInputArray("user_bundle_role_functions", node, req, newRecordId, authData);

     const insertResult = await AddUserBundleRoleFunctions(newRecordId, mutatedObj, req, authData);
