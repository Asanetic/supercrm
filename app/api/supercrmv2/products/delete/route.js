...
import { DeleteProducts } from '../products/ProductsDbGateway';
...
export async function GET(request) {
  ...
  const deleteToken = searchParams.get('_products_delete_record');
  const deleteTokenDecode = base64Decode(deleteToken);

  if (deleteToken) {
    // Customize table and WHERE clause here
    const table = 'products'; // Replace with your actual table
    
     
          
          const deleteAttachedMedia = await mosyQddata('products', 'primkey', deleteTokenDecode);
          
          const fileToDelete = deleteAttachedMedia?.product_image
         
          mosyDeleteFile(fileToDelete);
    
    const whereStr = `WHERE primkey = '${deleteTokenDecode}'`;

    const res = await DeleteProducts(deleteTokenDecode, whereStr);
