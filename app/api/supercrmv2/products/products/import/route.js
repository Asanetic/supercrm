...
import { AddProducts } from "../ProductsDbGateway";
...
export async function POST(req) {
  try {
    ...
    for (const node of processedInsertObj) {
      ...
      const mutatedObj = mutateInputArray("products", node, req, newRecordId, authData);

     const insertResult = await AddProducts(newRecordId, mutatedObj, req, authData);
