...
import { DeleteQuotations } from '../quotations/QuotationsDbGateway';
...
export async function GET(request) {
  ...
  const deleteToken = searchParams.get('_quotations_delete_record');
  const deleteTokenDecode = base64Decode(deleteToken);

  if (deleteToken) {
    // Customize table and WHERE clause here
    const table = 'quotations'; // Replace with your actual table
    
     
    
    const whereStr = `WHERE primkey = '${deleteTokenDecode}'`;

    const res = await DeleteQuotations(deleteTokenDecode, whereStr);
